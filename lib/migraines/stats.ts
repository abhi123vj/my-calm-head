...
export function monthlySeries(
  ...
  for (const episode of episodes) {
    ...
    if (bucket) bucket.push(severity ?? Number.NaN);
    else byMonth.set(key, [severity ?? Number.NaN]);
  }
  ...
  for (let offset = months - 1; offset >= 0; offset -= 1) {
    ...
    const recorded = values.filter((value) => !Number.isNaN(value));
    ...
    points.push({
      ...
      year: pointYear,
      count: values.length,
      averageSeverity: recorded.length > 0 ? mean(recorded) : null,
    });
  }
